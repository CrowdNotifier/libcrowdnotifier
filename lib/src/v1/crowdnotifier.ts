...
export class CryptoV1 {
  ...
  static scan(publicKey: Uint8Array,
      info: ILocationInfo,
      entry: number,
      departure: number,
  ...
  static match(visit: IVisit, exposure: ITrace): (undefined | string) {
    ...
    // As there is no UInt8Array.equals, use a string comparison on base64
    if (to_base64(visit.sharedKey) !== to_base64(diffieHellman)) {
      return undefined;
    }
    ...
    const [entryStr, departureStr] = msg.split('::', 2);
